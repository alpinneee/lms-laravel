...
<div class="space-y-6">
    ...
    <div>
        ...
        <p class="mt-1 text-sm text-gray-500">Update class details for {{ $course->course_name }}</p>
    </div>

    <!-- Form Card -->
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.courses.update-class', ['course' => $course, 'class' => $class]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="mt-1 text-sm text-gray-900 font-medium">{{ $class->class_id ?? 'CLS-' . $class->id }}</div>
                        </div>
                        
                        <!-- Quota -->
                        <div>
                            <label for="quota" class="block text-sm font-medium text-gray-700">Quota</label>
                            <input type="number" name="quota" id="quota" value="{{ old('quota', $class->quota) }}" min="1" 
                        ...
                        <div>
                            ...
                            <input type="number" name="duration_day" id="duration_day" value="{{ old('duration_day', $class->duration_day) }}" min="1" 
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500"
                                   required>
                            @error('duration_day')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="start_reg_date" id="start_reg_date" value="{{ old('start_reg_date', $class->start_reg_date->format('Y-m-d')) }}" 
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500"
                                       required>
                            ...
                            <div>
                                ...
                                <input type="date" name="end_reg_date" id="end_reg_date" value="{{ old('end_reg_date', $class->end_reg_date->format('Y-m-d')) }}" 
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500"
                                       required>
                                @error('end_reg_date')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $class->start_date->format('Y-m-d')) }}" 
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500"
                                       required>
                                @error('start_date')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input type="text" name="room" id="room" value="{{ old('room', $class->room) }}" 
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500"
                                   required>
                            @error('room')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                ...
                                @foreach($instructures as $instructor)
                                    <div class="flex items-center">
                                        <input type="checkbox" name="instructors[]" id="instructor_{{ $instructor->id }}" value="{{ $instructor->id }}" 
                                               ...
                                               {{ in_array($instructor->id, old('instructors', $selectedInstructors)) ? 'checked' : '' }}>
