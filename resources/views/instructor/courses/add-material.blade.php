...
<div class="space-y-3">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('instructor.courses.materials', $course) }}" class="bg-gray-600 text-white px-3 py-1.5 rounded text-xs hover:bg-gray-700">
            Back
        </a>
    </div>
    ...
    <div class="bg-white shadow rounded border p-3">
        ...
        <div class="text-xs text-gray-600">
            Class: {{ $class->start_date->format('j M') }} - {{ $class->end_date->format('j M Y') }} | {{ $class->location }}
        </div>
    </div>

    <!-- Add Material Form -->
    <div class="bg-white shadow rounded border p-4">
        <form method="POST" action="{{ route('instructor.courses.store-material', [$course, $class]) }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('instructor.courses.materials', $course) }}" class="bg-gray-300 text-gray-700 px-4 py-2 rounded text-sm hover:bg-gray-400">
                    Cancel
                </a>
            </div>
        </form>
